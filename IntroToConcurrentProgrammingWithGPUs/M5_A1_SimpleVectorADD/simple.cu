...
// Computes the vector product of A and B into C. The 3 vectors have the same number of elements numElements.
...
void allocateHostMemory(int numElements, float **h_A, float **h_B, float **h_C) {
...
void allocateDeviceMemory(int numElements, float **d_A, float **d_B, float **d_C) {
    size_t size = numElements * sizeof(float);
    cudaError_t err;
...
int main(void) {
    ...
    float *h_A, *h_B, *h_C;
    allocateHostMemory(numElements, &h_A, &h_B, &h_C);

    float *d_A, *d_B, *d_C;
    allocateDeviceMemory(numElements, &d_A, &d_B, &d_C);
