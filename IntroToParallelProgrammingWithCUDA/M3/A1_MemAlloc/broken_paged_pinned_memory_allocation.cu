...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    {
        // Use the original range but modify assignment
        // Base values: use smaller portion of range for bases
        h_a[i] = 1.0f + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/9.0f));
        
        // Exponent values: use much smaller range to prevent overflow
        h_b[i] = static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/3.0f));
    }

    return {h_a, h_b};
}

// Fix in readCsv function:
__host__ std::tuple<float *, float *, int>readCsv(std::string filename)
{
    ...
    float *h_a;
    h_a = (float *)malloc(size);  // Fix: should be (float *), not (float **)
    ...
    float *h_b;
    cudaMallocHost((float **)&h_b, size);  // Fix: should be cudaMallocHost, not cudaMallocMost
...
{
    size_t size = numElements * sizeof(float);  // Fix: should be float, not int

    cudaError_t err = cudaMemcpy(d_a, h_a, size, cudaMemcpyHostToDevice);  // Fix: should be cudaMemcpy, not cudaGMemcpy
...
__host__ void executeKernel(float *d_a, float *d_b, float *h_c, int numElements, int threadsPerBlock)
{
    ...
    pow<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, h_c, numElements);  // Fix: should call pow, not div
