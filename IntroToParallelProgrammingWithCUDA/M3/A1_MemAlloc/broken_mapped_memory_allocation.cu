...
{
    // For mapped memory, we don't allocate separate device memory
    // The device pointers will be obtained from host mapped memory
    // Return NULL pointers that will be set later
    float *d_a = NULL;
    float *d_b = NULL;
    return {d_a, d_b};
}
...
{
    // For mapped memory, we get device pointers from host mapped memory
...
__host__ void executeKernel(float *d_a, float *d_b, float *h_c, int numElements, int threadsPerBlock)
{
    ...
    int blocksPerGrid =(numElements + threadsPerBlock - 1) / threadsPerBlock;  // Fix: should divide, not add
    div<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, h_c, numElements);  // Fix: should call div, not add
...
__host__ void deallocateMemory(float *d_a, float *d_b) //Should not free mapped memory as we only point them to the host memory
{
...
int main(int argc, char *argv[])
{
    ...
    // For mapped memory, we need to get device pointers properly
    float *d_a = NULL;
    float *d_b = NULL;
    ...
    cudaError_t err = cudaHostGetDevicePointer((void **)&d_a, (void *)h_a, 0);
    if (err != cudaSuccess) {
        fprintf(stderr, "Failed to get device pointer for h_a\n");
        exit(EXIT_FAILURE);
    }
    
    err = cudaHostGetDevicePointer((void **)&d_b, (void *)h_b, 0);
    if (err != cudaSuccess) {
        fprintf(stderr, "Failed to get device pointer for h_b\n");
        exit(EXIT_FAILURE);
    }
    ...
    // No need to deallocate device memory for mapped memory
    // deallocateMemory(d_a, d_b);  // Comment this out
