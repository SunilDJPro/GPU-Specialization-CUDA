...
__global__ void constantMemorySearch(int *input, int totalFound)
{
    int threadId = blockDim.x * blockIdx.x + threadIdx.x;
...
__host__ void copyFromHostToDevice(std::string kernelType, int *input, int numElements, int numThreads, int *d_input)
{
    ...
    if(!strcmp(kernelType.c_str(), "constant"))
    {
        ...
        cudaError_t err1 = cudaMemcpyToSymbol(constant_num_elements, &numElements, sizeof(int), 0, cudaMemcpyHostToDevice);
        cudaError_t err2 = cudaMemcpyToSymbol(constant_num_threads, &numThreads, sizeof(int), 0, cudaMemcpyHostToDevice);
        cudaError_t err3 = cudaMemcpyToSymbol(constant_thread_span, &threadSpan, sizeof(int), 0, cudaMemcpyHostToDevice);
        ...
        // Still need to copy the actual data to global memory (constant memory holds parameters only)
